...
(* let rec zip (x : int list) (y : int list) : (int * int) list =
  ...
  | [a] -> (match y with
           | [b] -> [(a, b)]
           | _ ->  raise (Invalid_argument "different length"))
  | hdx :: tlx -> (match y with
                | [c] -> raise (Invalid_argument "different length")
                | [] -> raise (Invalid_argument "different length")
...
let rec zip (x : int list) (y : int list) : (int * int) list =
  ...
  | [], [] -> []
  | _, _ -> raise (Invalid_argument "different length");;
